...
import { generateChunkMeshPure, MeshGenerationConfig, PureMeshData } from './pureMesh';
import * as PureNoise from './pureNoise';
import * as PureTerrain from './pureTerrain';
...
@ccclass('PureWorld')
export class PureWorld extends Component {
    ...
    // Cache block definitions for worker
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    private generateMeshFallback(chunkData: ChunkData): PureMeshData | null {
        try {
            ...
            // Convert MeshData to PureMeshData format
            return {
    ...
    private createBatchedMesh(chunks: { chunkData: ChunkData; meshData: PureMeshData }[]): void {
        ...
        batchNode.setPosition(0, 0, 0); // Vertices already have world positions
    ...
    /**
     * Process mesh update queue (call from update loop).
     */
