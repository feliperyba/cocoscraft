...
export class PassContext {
    ...
    blitScreen (passIdx = 0): PassContext {
        // [CocosCraft Fix] Guard against materials whose effect asset hasn't loaded yet.
        // When the effect is not registered, Material.initialize() produces empty passes[],
        // and blit.material.passes[passIdx] returns undefined, crashing executor.ts:490.
        // Skip the blit entirely; once the effect loads, subsequent frames will include it.
        const mat = this.material;
        if (mat && mat.passes.length > passIdx) {
